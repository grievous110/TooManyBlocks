...
static void OpenGLDebugCallback(
    ...
    switch (severity) {
        case GL_DEBUG_SEVERITY_HIGH: lgr::lout.error(logStream.str()); break;
        case GL_DEBUG_SEVERITY_MEDIUM: lgr::lout.warn(logStream.str()); break;
        case GL_DEBUG_SEVERITY_LOW: lgr::lout.info(logStream.str()); break;
        default: lgr::lout.debug(logStream.str()); break;
    }
}
...
bool GLLogCall(const char* functionName, const char* file, int line) {
    ...
    while ((error = glGetError()) != GL_NO_ERROR) {
        ...
        stream << "[OpenGL Error " << toGLErrorString(error) << " 0x" << std::hex << error << "] caused by "
               << functionName << " in " << file << " line " << std::dec << line;
...
size_t GLsizeof(unsigned int type) {
    switch (type) {
        ...
        default: throw std::runtime_error("Unknown component type in GLTF accessor.");
    }
}